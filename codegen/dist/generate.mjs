...
function generateDeviceType(device) {
  let out = `DeviceBase & {
    type: '${device.id}';
...
function generateOption(option) {
  ...
  switch (option.type) {
    case "select":
      out = option.props.options.map((option2) => literalType(option2.value)).join(" | ");
      break;
    case "textInput":
      out = "string";
      break;
  ...
  if ("props" in option && "allowEmpty" in option.props && option.props.allowEmpty) {
    out += " | undefined";
  }
  return out;
}
function generateCodeGrids(device) {
...
function generateCodeGridType(trigger) {
  return `{
    type: '${trigger.type}';
    blocks: Program;
  }`;
}
function generateDefaultDeviceOptions(device) {
  let out = "{";
...
function generateDefaultOption(option) {
  switch (option.type) {
    case "select":
    ...
    case "color":
      return option.defaultValue == void 0 ? "undefined" : `"${option.defaultValue}"`;
    case "terrainId":
      return option.defaultValue == void 0 ? "undefined" : `"${option.defaultValue}"`;
    case "itemId":
      return option.defaultValue == void 0 ? "undefined" : `"${option.defaultValue}"`;
...
writeFileSync(
  ...
  generateDeviceTypes(deviceOptions)
);
